...
pub async fn ingest_sha(state: &AppState, sha: &str, message: Option<&str>) -> Result<Option<i64>, String> {
    ...
    for plan in &ingested.desires {
        ...
        if let Err(e) =
            notify::push_host_status(state, ingested.build_id, &plan.host, build_state, None, &description)
                .await
        {
...
mod tests {
    ...
    fn parses_build_paths() {
        ...
        assert_eq!(map.get("nixlaptop").map(String::as_str), Some("/nix/store/abc"));
        assert_eq!(map.get("nixserver").map(String::as_str), Some("/nix/store/def"));
    }
