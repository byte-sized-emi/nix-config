...
pub async fn log_stream(
  ...
  let existing = state.db.logs_after(id, after, 10_000).await.unwrap_or_default();
        ...
        match rx.recv().await {
          ...
          Err(broadcast::error::RecvError::Lagged(_)) => {
            if let Ok(missed) = state.db.logs_after(id, last_id, 10_000).await {
              catchup = missed.into_iter().map(|e| (e.id, e.ts, e.line)).rev().collect();
            }
          }
  ...
  let heartbeat = stream::unfold(tokio::time::interval(Duration::from_secs(15)), |mut ticker| async move {
    ticker.tick().await;
