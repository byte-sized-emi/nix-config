...
pub async fn poll(
  ...
  state
    .db
    .update_host_seen(&name, req.actual_derivation.as_deref(), req.actual_commit.as_deref())
    .await
...
pub async fn claim(
  ...
  state
    ...
    .await
    .map_err(ApiError::internal)?
    .ok_or_else(|| ApiError::bad_request(format!("no known build of {name} at commit {}", req.commit)))?;
...
pub async fn clear_desired(State(state): State<AppState>, Path(name): Path<String>) -> Result<StatusCode, ApiError> {
  state.db.clear_desired_override(&name).await.map_err(ApiError::internal)?;
  Ok(StatusCode::OK)
}
...
pub async fn log(
  ...
  let lines: Vec<String> = req.lines.into_iter().filter(|line| !line.trim().is_empty()).collect();
  if lines.is_empty() {
    return Ok(StatusCode::OK);
  }
  let entries = state.db.append_logs(id, &lines).await.map_err(ApiError::internal)?;
...
pub async fn stream_log(
  ...
  while let Some(chunk) = stream.next().await {
    ...
    while let Some(offset) = buffer[start..].iter().position(|&b| b == b'\n') {
      ...
      lines.push(String::from_utf8_lossy(&buffer[start..end]).trim_end_matches('\r').to_string());
      start = end + 1;
    }
    buffer.drain(..start);
    let lines: Vec<String> = lines.into_iter().filter(|line| !line.trim().is_empty()).collect();
    if !lines.is_empty() {
      let entries = state.db.append_logs(id, &lines).await.map_err(ApiError::internal)?;
      let channel = state.channel(id).await;
      for entry in entries {
  ...
  let tail = String::from_utf8_lossy(&buffer).trim_end_matches('\r').trim().to_string();
  if !tail.is_empty() {
    let entries = state.db.append_logs(id, &[tail]).await.map_err(ApiError::internal)?;
    let channel = state.channel(id).await;
    for entry in entries {
      let _ = channel.send((entry.id, entry.ts, entry.line));
    }
...
pub async fn finish(
  ...
  let status = match req.status.as_str() {
    "succeeded" => "succeeded",
    "failed" => "failed",
    _ => return Err(ApiError::bad_request("status must be 'succeeded' or 'failed'")),
  };
  state
    .db
    ...
    .await
    .map_err(ApiError::internal)?;
  state.db.set_host_status(&dep.host, "idle").await.map_err(ApiError::internal)?;
  let build = state.db.build_by_id(dep.build_id).await.map_err(ApiError::internal)?;
  if status == "succeeded"
      ...
      .map_err(ApiError::internal)?
  {
    state
      .db
      .set_host_actual(&dep.host, &deriv, build.as_ref().map(|b| b.commit.as_str()).unwrap_or(""))
      .await
      .map_err(ApiError::internal)?;
  }
  let (state_str, description) = if status == "succeeded" {
    ...
    ("failure", req.error.as_deref().unwrap_or("deployment failed").to_string())
  };
  if let Err(e) = notify::push_host_status(&state, dep.build_id, &dep.host, state_str, Some(id), &description).await {
