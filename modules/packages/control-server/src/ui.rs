...
fn short(s: &str, len: usize) -> String {
    // Truncate by characters, not bytes, so this never panics on a multi-byte
    // UTF-8 boundary (commit SHAs and derivations are ASCII today, but this
